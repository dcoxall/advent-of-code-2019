...
func getResult(prog: openArray[int64], vector: Vector2D): int =
  var intcode = initProgram(prog)
  intcode.execute()
  intcode.resume(int64(vector.x), int64(vector.y))
  for resp in intcode:
    return int(resp)

# challenge code here
# ===================
let file = open("./inputs/19.txt")

var
  y, x: int
  input = strip(readAll(file)).split({ ',' }).mapIt(int64(parseInt(it)))

while getResult(input, (x + 99, y)) == 0:
  y += 1
  while getResult(input, (x, y + 99)) == 0:
    x += 1
